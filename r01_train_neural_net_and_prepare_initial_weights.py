...
# os.chdir("C:\\Users\\teddy\\Downloads\\HLIMDS_LR04\\")
...
def read_additional_dataset_images(type, full_size=False):
    ...
    for f in files:
        ...
        if full_size is True:
            ...
            output_image = Image.open(f).resize((28,28)).convert('RGBA')
            background = Image.new('RGBA', output_image.size, (255, 255, 255))
            output_image = Image.alpha_composite(background, output_image).convert('L')
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(final_model_path) or continue_training == 1:
        ...
        learning_rate = 0.001
        if optimizer == 'SGD':
            optim = SGD(lr=learning_rate, decay=1e-6, momentum=0.1, nesterov=True)
        else:
        ...
        score = model.evaluate_generator(generator=evaluate_generator_train(X_test, Y_test, batch_size),
